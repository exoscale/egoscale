...
var privnetListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		table.SetHeader([]string{"zone", "Name", "ID", "Associated Virtual machine", "StartIP", "EndIP", "Netmask"})
...
func listPrivnets(zone string, table *table.Table) error {
	...
	if zone != "" {
		...
		for i, pNet := range pns {
			...
			table.Append([]string{zone, pn.Name, pn.ID.String(), vmNum, pn.StartIP.String(), pn.EndIP.String(), pn.Netmask.String()})
